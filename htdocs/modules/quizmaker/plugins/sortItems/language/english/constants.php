<?php
   /**
 * Name: modinfo.php
 * Description:
 *
 * You may not change or alter any portion of this comment or credits
 * of supporting developers from this source code or any supporting source code
 * which is considered copyrighted (c) material of the original comment or credit authors.
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 * @copyright The XOOPS Project http://sourceforge.net/projects/xoops/
 * @license http://www.fsf.org/copyleft/gpl.html GNU public license
 * @package : XOOPS
 * @Module : 
 * @subpackage : Menu Language
 * @since 2.5.7
 * @author Jean-Jacques DELALANDRE (jjdelalandre@orange.fr)
 * @version {version}
 * Traduction:  
 */
 
defined( 'XOOPS_ROOT_PATH' ) or die( 'Accès restreint' );

//------------------------------------------------------------------
define('_QT_QUIZMAKER_LISTBOXSORTITEMS', "Trier une listbox");
define('_QT_QUIZMAKER_LISTBOXSORTITEMS_DESC', "Ce slide est composé d'une liste et de boutons de déplacement.<br>Le tri peut être inverse.");
define('_QT_QUIZMAKER_LISTBOXSORTITEMS_CONSIGNE', "Vous devez trier la liste selon les critères énoncés dans la question.<br>Sélectionnez un item puis cliquez sur les boutons latéraux pour le déplacer vers le haut ou vers le bas.<br>Selon les options choisies, la liste doit être triée en ordre strict ou non, c'est à dire que l'ordre peut être inverse.");

?>
