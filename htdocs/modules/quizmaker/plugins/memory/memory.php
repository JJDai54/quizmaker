...
* *********************************************************** */
 	public function getFormOptions($caption, $optionName, $jsonValues = null, $folderJS = null)
 	{
      ...
      $inpClasse = new \XoopsFormSelect(_AM_QUIZMAKER_VARIANT, "{$optionName}[{$name}]", $variant);
      ...
      if ($variant == 'grille'){
          ...
          $inpGap->setMinMax(0, 12, _AM_QUIZMAKER_UNIT_PIXELS, "1");
      ...
      $name = 'tempo';  
      $inpDoublons = new \XoopsFormNumber(_AP_QUIZMAKER_TEMPO,  "{$optionName}[{$name}]", $this->lgPoints, $this->lgPoints, $tValues[$name]);
      $inpDoublons->setMinMax(0, 5, _AM_QUIZMAKER_UNIT_SECONDS, "0.5");
      $inpDoublons->setDescription(_AP_QUIZMAKER_TEMPO_DESC);
      $trayOptions->addElementOption($inpDoublons);     
      ...
      $inpMaxAttemps->setMinMax(0, 100, _AM_QUIZMAKER_UNIT_ATTEMPTS);
