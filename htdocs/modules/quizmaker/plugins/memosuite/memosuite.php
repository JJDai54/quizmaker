...
var $messagegArr = array();
	...
	public function __construct()
	{
        ...
        //--------------------------------------------------------------
        $this->addMessages(['ready_btn', 'ready', 'player_turn','game_failure', 'next_slide_btn']); 
        
/*
        $this->addMessages(['ready_btn', 'ready', 'player_turn','game_failure', 'getSeqence','noSequence','attemptsOut' , 'attemptsNum']); //
...
* *********************************************************** */
 	public function getFormOptions($caption, $optionName, $jsonValues = null, $folderJS = null)
 	{
      ...
      $trayOptions = $this->getNewXFTableOptions($caption);  
      //--------------------------------------------------------------------           
      $variantsArr = ['grille' => _AM_QUIZMAKER_IMG_GRID,
                      'liste'   => _AM_QUIZMAKER_IMG_LIST];
      ...
      $name = 'gridCols';  
      $inpGridCols = new \XoopsFormNumber(_AP_QUIZMAKER_COLS,  "{$optionName}[{$name}]", $this->lgPoints, $this->lgPoints, $tValues[$name]);
      $inpGridCols->setMinMax(1, 16, _AM_QUIZMAKER_UNIT_COLUMNS, "1");
      $inpGridCols->setDescription(_AP_QUIZMAKER_COLS_DESC);
      $trayOptions->addElementOption($inpGridCols);    
...
*/      
      ...
      $name = 'sequenceTempo';  
      $inpSequenceTempo = new \XoopsFormNumber(_LG_PLUGIN_MEMOSUITE_TEMPO_SUITE,  "{$optionName}[{$name}]", $this->lgPoints, $this->lgPoints, $tValues[$name]);
      $inpSequenceTempo->setMinMax(0.2, 5, _AM_QUIZMAKER_UNIT_SECONDS, "0.1");
      $inpSequenceTempo->setDescription(_LG_PLUGIN_MEMOSUITE_TEMPO_SUITE_DESC);
      $trayOptions->addElementOption($inpSequenceTempo);     

       //--------------------------------------------------------------
      //$trayOptions->insertBreak(sprintf(QUIZMAKER_OPTIONS_BREAK_STYLE, _LG_PLUGIN_MEMOSUITE_OPTIONS_GAME));  
       //--------------------------------------------------------------
      $name = 'retryMode';
      $inRetryMode = new \XoopsFormSelect(_LG_PLUGIN_MEMOSUITE_MODE, "{$optionName}[{$name}]", $tValues[$name]);
      $inRetryMode->setDescription(_LG_PLUGIN_MEMOSUITE_MODE_DESC);
      $inRetryMode->addOption(0, _LG_PLUGIN_MEMOSUITE_MODE_0);
      $inRetryMode->addOption(1, _LG_PLUGIN_MEMOSUITE_MODE_1);
      $inRetryMode->addOption(2, _LG_PLUGIN_MEMOSUITE_MODE_2);
      $trayOptions->addElementOption($inRetryMode);    
      ...
      $name = 'GridBackground';  
      $inpGridBackground = new XoopsFormColorPicker(_AP_QUIZMAKER_BACKGROUND, "{$optionName}[{$name}]", $tValues[$name]);
      $trayOptions->addElementOption($inpGridBackground);   
