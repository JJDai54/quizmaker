 ...
 */
class slide_sortCombobox extends XoopsModules\Quizmaker\Type_question
{
	...
	{
        parent::__construct("sortCombobox");
    }
...
* *********************************************************** */
 	...
 	{    
      ...
      $input->addOption("N", "Uniquement l'ordre indiqué dans la quesion");            
      $input->addOption("R", "Aurorise l'ordre inverse");            
...
//    echo "<hr>answers<pre>" . print_r($answers, true) . "</pre><hr>";
        ...
        for ($h = 0; $h < $this->maxPropositions; $h++){
            ...
            $inpLab  = new XoopsFormLabel("", h+1 . " : Titre de la liste");  
            $trayCaption->addElement($inpLab);
            
            $name = $this->getName($h, 'caption');
            $inpCaption = new \XoopsFormText("", $name, $this->lgProposition, $this->lgProposition, $caption);
