...
//jjdai : verif email
define('_US_ATTEMPTED_INTRUSION', "Tentative d'intrusion");
define('_US_ATTEMPTED_INTRUSION_REJECT', "Tentative d'intrusion");
define('_US_ATTEMPTED_INTRUSION_INVALID_DOMAIN', "Tentative d'intrusion par : %s.<br>Le domain %s n'est pas valide");

function isValidDomain($email, $bEcho = false) {
    $email = idn_to_ascii($email);
    $domain = substr(strrchr($email, "@"), 1) . '.';
    $bolOk = checkdnsrr($domain, "MX") || checkdnsrr($domain, "A");
    if ($bEcho) echo "email : {$email} - domaine : {$domain} => " . (($bolOk) ? "ok" : "not valid") . "<br>";           
    return $bolOk;
}
...
if($email && !isValidDomain($email, true)) {
    $ip = XoopsUserUtility::getIP(true);
    $msg = _US_ATTEMPTED_INTRUSION_REJECT . "<br>IP : {$ip}";
//======================================================
...
/*
            ...
            //echo implode(',', $GLOBALS['xoopsConfigUser']['activation_group']) . "<br>";
            echo "groups activation : " . $GLOBALS['xoopsConfigUser']['activation_group']*99 . "<br>";
            //$xoopsMailer->setToUsers($GLOBALS['xoopsConfig']['adminmail']);
            

            $xoopsMailer->setFromEmail($GLOBALS['xoopsConfig']['adminmail']);
            $xoopsMailer->setFromName($GLOBALS['xoopsConfig']['sitename']);
            $xoopsMailer->setSubject(_US_ATTEMPTED_INTRUSION);
            $xoopsMailer->setBody(sprintf(_US_ATTEMPTED_INTRUSION, $newuser->getVar('uname'),$newuser->getVar('email')));
            $xoopsMailer->send(true);
*/            
/*
*/
$email = 'jjdelalandre@orange.fr';
$subject = _US_ATTEMPTED_INTRUSION;
$body = sprintf(_US_ATTEMPTED_INTRUSION, $newuser->getVar('uname'),$newuser->getVar('email'));
$headers = null;
    $xoopsMailer->sendMail($email, $subject, $body, $headers);
   
//======================================================
    redirect_header(XOOPS_URL,5, $msg);
}
 // pas glop : 2a01:cb04:286:4200:908d:d94b:f971:54ea  
...
if ($current_step > 0 && empty($stop) && (!empty($steps[$current_step - 1]['step_save']) || !isset($steps[$current_step]))) {
    ...
    if (empty($stop)) {
        ...
        if (!isValidDomain($newuser->getVar('email'), false)){
  $debugErr = false;
  if($debugErr){
  global $xoopsLogger;
  $xoopsLogger->renderingEnabled = true;
...
*/
            ...
            $stop .= "Bad domain ou tentavive d'intrusion :-((<br>";
...
//exit;
        ...
        else if (!$member_handler->insertUser($newuser)) {
