...
$userEmail   = Request::getString('user_email', '');
...
//echoGPF('GP');
    ...
    $templateMain = 'quizmaker_admin_players.tpl';
        ///-------------------------------------------------------
...
//         $inpUsers = new XoopsFormSelectUser('joueurs', 'user_id', true, $userId);
        
        if($domaine == "cookies"){

            //$userArr = $cookiesHandler->getListKeyName(null, 'cookie_uid,cookie_email', false);
            $userArr = $cookiesHandler->getListKeyName(null, 'cookie_email','cookie_uname', false, true);
        }else{
            //$userArr = $resultsHandler->getListKeyName(null, 'result_email,result_email', false);
            $userArr = $resultsHandler->getListKeyName(null, 'result_email','result_uname', false, true);
        }

        $inpUsers = new \XoopsFormSelect(_AM_QUIZMAKER_PLAYER, 'user_email', $userEmail);        
        $inpUsers->addOptionArray($userArr);
        $inpUsers->setExtra('onchange="document.quizmaker_select_filter.sender.value=this.name;document.quizmaker_select_filter.submit();"'.FQUIZMAKER\getStyle(QUIZMAKER_BG_LIST_CAT));
//    public function &getAll(CriteriaElement $criteria = null, $fields = null, $asObject = true, $id_as_key = true)
         $GLOBALS['xoopsTpl']->assign('inpUsers', $inpUsers->render());
...
//             $permClear = $clPerms->getPermissions('clearcookies_quiz', $catId);
             include_once('players-cookies.php');
         }else{
//             $permClear = $clPerms->getPermissions('clearresults_quiz', $catId);
             include_once('players-results.php');
         }
