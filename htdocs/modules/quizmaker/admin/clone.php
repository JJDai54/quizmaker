...
$templateMain = 'quizmaker_admin_clone.tpl';
...
switch ($op) {
    ...
    default:
        ...
        $form  = new \XoopsThemeForm(\sprintf(\_AM_QUIZMAKER_CLONE_TITLE, $quizmakerHelper->getModule()->getVar('name', 'E')), 'clone', 'clone.php', 'post', true);
        $clone = new \XoopsFormText(\_AM_QUIZMAKER_CLONE_NAME, 'clone', 20, 20, '');
        $clone->setDescription(\_AM_QUIZMAKER_CLONE_NAME_DSC);
    ...
    case 'submit':
        ...
        if ($clone && \is_dir($dirClone)) {
            \redirect_header('clone.php', 3, \sprintf(\_AM_QUIZMAKER_CLONE_EXISTS, $clone));
        }

        $patterns = [
            \mb_strtolower(\QUIZMAKER_DIRNAME)          => \mb_strtolower($clone),
            \mb_strtoupper(\QUIZMAKER_DIRNAME)          => \mb_strtoupper($clone),
            \ucfirst(\mb_strtolower(\QUIZMAKER_DIRNAME)) => \ucfirst(\mb_strtolower($clone)),
        ];
        ...
        if (\is_dir($GLOBALS['xoops']->path('modules/' . \mb_strtolower($clone)))) {
            $msg .= \sprintf(\_AM_QUIZMAKER_CLONE_CONGRAT, "<a href='" . \XOOPS_URL . "/modules/system/admin.php?fct=modulesadmin&op=installlist'>" . \ucfirst(\mb_strtolower($clone)) . '</a>') . "<br>\n";
            if (!$logocreated) {
                $msg .= \_AM_QUIZMAKER_CLONE_IMAGEFAIL;
            }
        } else {
            $msg .= \_AM_QUIZMAKER_CLONE_FAIL;
        }
